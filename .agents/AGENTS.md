...
- Tier 3: `clients/web/*`, `clients/composeApp`, mobile hosts
  Transport rules: web clients use the HTTP gateway only; mobile clients use the Rust CLI bridge
  only; CLI/operator flows may access runtime capabilities directly.
  Rust extension points in `clients/agent-runtime/src/` are trait-based: `Provider`, `Channel`,
  `Tool`, `Memory`, `Observer`, `SecurityPolicy` under `providers/`, `channels/`, `tools/`,
  `memory/`, `observability/`, and `security/`.
...
When working with Rust in this repo, prefer these skills:

- `rust` — base Rust guidance: ownership, borrowing, errors, traits, modules, testing,
  async boundaries, and validation habits.
- `corvus-rust-runtime` — Corvus-specific runtime guidance for `clients/agent-runtime`, including
  trait-driven architecture, security boundaries, factory wiring, and repo validation commands.

Recommended loading order:

1. load `rust` for any Rust task
2. add `corvus-rust-runtime` for changes under `clients/agent-runtime`
...
- Web code follows `clients/web/biome.json`: spaces, width 100, double quotes, trailing commas
  `es5`.
  Kotlin:
...
- Use `tasks.register`, not eager task creation; use `configureEach`, not `all`; never use
  `afterEvaluate`.
- Keep dependencies in `gradle/libs.versions.toml` and reusable Gradle conventions in
  `gradle/build-logic/`.
