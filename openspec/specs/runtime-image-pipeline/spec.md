...
Provider dispatch MUST consume the entire `&[StagedImage]` slice for an admitted turn, MUST
preserve image order, and MUST NOT collapse a multi-image turn to only the first staged image.
...
2. **Gate**: Apply config-driven admission control before any fetch:
    ...
    - Image count MUST NOT exceed the effective `multimodal.max_images_per_turn` value (reject with
      `TooManyImages` otherwise)
...
During handoff, the runtime MUST attach all admitted staged images for the last user turn to the
provider request while preserving order.
...
If `multimodal.max_images_per_turn` is omitted, the runtime MUST default to 4. The runtime MUST
...
| `TooManyImages`        | "Too many images ({count}). Maximum {limit} per message."  | Image count exceeds the effective `multimodal.max_images_per_turn` |
...
For configurable limits, the `TooManyImages` message MUST report the attempted count and effective
configured limit for the rejected turn.
...
max_images_per_turn = null                        # optional integer — override effective per-turn image-count limit
...
### REQ-9: Observability for Multi-Image Runtime Dispatch
...
provider-sent, and provider-error outcomes MUST remain metadata-only and MUST distinguish
multi-image turns from single-image turns without dropping later-image context.
...
### REQ-10: Regression Coverage for Multi-Image Runtime Behavior
