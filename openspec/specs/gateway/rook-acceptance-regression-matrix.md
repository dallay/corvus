...
| #594 | usage placeholder, settings | `pnpm --dir clients/web --filter @corvus/rook-dashboard run build`; `pnpm --dir clients/web --filter @corvus/rook-dashboard run test`; `pnpm --dir clients/web --filter @corvus/rook-dashboard run test:e2e`; `pnpm --dir clients/web --filter @corvus/rook-dashboard run check` | none | `dashboard/spec.md` | openspec/changes/archive/2026-04-22-rook-594-dashboard-usage-settings/verify-report.md | Auto | Usage remains placeholder-only by contract; logs/backups remain deferred outside this slice. |
