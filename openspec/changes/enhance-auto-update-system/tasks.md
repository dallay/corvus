...
- [ ] 1.1 Add RED unit tests in `clients/agent-runtime/src/config/schema.rs` for new update
  ...
  behavior.
- [ ] 1.2 Implement GREEN schema updates in `clients/agent-runtime/src/config/schema.rs` for
  ...
  validation.
- [ ] 1.3 Add RED unit tests in `clients/agent-runtime/src/update/mod.rs` for policy resolution,
  install method precedence (`override -> detected -> unknown`), and install/check state transition
  invariants.
- [ ] 1.4 Implement GREEN core model/types in `clients/agent-runtime/src/update/mod.rs` (
...
- [ ] 2.1 Add RED CLI command tests for `update status`, `update check`, and `update install`
  deterministic output/exit semantics in runtime command test coverage associated with
  `clients/agent-runtime/src/main.rs`.
- [ ] 2.2 Implement GREEN `update status|check|install` command wiring and exit code mapping in
  `clients/agent-runtime/src/main.rs`, routed through `UpdateManager` entrypoints.
- [ ] 2.3 Add RED concurrency/resilience tests in `clients/agent-runtime/src/update/mod.rs` (or
...
- [ ] 2.4 Implement GREEN file-lock and atomic persistence flow in
...
- [ ] 2.5 Implement deterministic install-method execution routing and unsupported fallback
...
- [ ] 3.1 Add RED tests in `clients/agent-runtime/src/channels/mod.rs` for channel visibility gating
  and canonical update payload parity with CLI status.
- [ ] 3.2 Implement GREEN channel integration in `clients/agent-runtime/src/channels/mod.rs` so
  opportunistic mentions and nonce-confirm flow use canonical status/policy gates.
- [ ] 3.3 Add RED daemon watcher tests in `clients/agent-runtime/src/daemon/mod.rs` for check
  interval behavior, deduped notifications, and policy-aware fan-out.
- [ ] 3.4 Implement GREEN daemon updater integration in `clients/agent-runtime/src/daemon/mod.rs`
  using canonical update payload and shared manager APIs.
- [ ] 3.5 Add RED admin contract tests in `clients/agent-runtime/src/gateway/admin.rs` and
...
- [ ] 3.6 Implement GREEN admin API and dashboard type updates in
  ...
  discipline.
- [ ] 3.7 Add RED/GREEN tasks in `clients/agent-runtime/src/main.rs` and
...
- [ ] 4.1 Add RED verification tests in `clients/agent-runtime/src/update/mod.rs` for
  checksum-required artifact paths, missing metadata failures, digest mismatch failures, and
  fail-closed install blocking.
- [ ] 4.2 Implement GREEN verification gate and structured verification/install audit event
  recording in `clients/agent-runtime/src/update/mod.rs`.
- [ ] 4.3 Add RED tests for `update history` ordering and schema expectations, then implement GREEN
...
- [ ] 4.4 Add RED restart-policy integration tests in `clients/agent-runtime/src/service/mod.rs` and
...
- [ ] 4.5 Refactor duplicated audit/restart decision code in
...
- [ ] 5.1 Add/update focused integration tests under `clients/agent-runtime/tests/` for full command
  contract coverage (`status|check|install|auto-enable|auto-disable|history`) and concurrency
  outcomes.
- [ ] 5.2 Add/update integration tests under `clients/agent-runtime/tests/` for cross-surface
  consistency (CLI status vs admin payload vs channel/daemon notification facts).
- [ ] 5.3 Run targeted runtime verification (`cargo test -p agent-runtime update`) and dashboard
  type/build checks for `clients/web/apps/dashboard/src/types/admin-config.ts`, fixing regressions
  in touched files.
- [ ] 5.4 Run full repository regression (`make test` and `make build`) and confirm every scenario
