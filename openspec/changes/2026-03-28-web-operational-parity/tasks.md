...
- [ ] 1.1 Extend `AdminConfigView` in `types/admin-config.ts` with typed interfaces for
...
- [ ] 1.2 Extend `AdminConfigForm` and `AdminConfigSnapshot` with form fields for new sections
...
- [ ] 1.3 Extend `AdminConfigUpdateRequest` with patch types for new sections
...
- [ ] 1.4 Extend `ConfigSection` type with new section variants
...
- [ ] 1.5 Create `ProviderPoolsSettings.vue` component
...
- [ ] 1.6 Create `ProviderPoolsSettings.spec.ts` unit tests
...
- [ ] 1.7 Create `UpdateSettings.vue` component
...
- [ ] 1.10 Create `WebSearchSettings.spec.ts` unit tests
    - **Acceptance**: Tests cover render, edit, and secret field semantics
    - **Spec ref**: REQ-10

- [ ] 1.11 Create `BrowserSettings.vue` component
...
- [ ] 1.12 Create `BrowserSettings.spec.ts` unit tests
    - **Acceptance**: Tests cover render and secret field semantics
    - **Spec ref**: REQ-10

- [ ] 1.13 Create `ComposioSettings.vue` component
...
- [ ] 1.14 Create `ComposioSettings.spec.ts` unit tests
    - **Acceptance**: Tests pass
    - **Spec ref**: REQ-10

- [ ] 1.15 Create `MemorySettings.vue` component
...
- [ ] 1.16 Create `MemorySettings.spec.ts` unit tests
    - **Acceptance**: Tests pass
    - **Spec ref**: REQ-10

- [ ] 1.17 Create `IdentitySettings.vue` component
...
- [ ] 1.18 Create `IdentitySettings.spec.ts` unit tests
    - **Acceptance**: Tests pass
    - **Spec ref**: REQ-10

### 1C: Dashboard Integration

- [ ] 1.19 Update `App.vue` to render new config sections
...
- [ ] 1.20 Update `configPayload.ts` to build payloads for new sections
...
- [ ] 1.21 Update `useConfig.ts` composable for new sections
...
- [ ] 1.22 Verify existing dashboard tests still pass
...
- [ ] 2.1 Add `AdminChannelStatusView` and `ChannelHealth` types to `gateway/admin.rs`
    - Define structs per design interface contracts
    - **Acceptance**: Types compile; `ChannelHealth` serializes to lowercase strings
    - **Spec ref**: REQ-2

- [ ] 2.2 Implement `handle_admin_channels` handler in `gateway/admin.rs`
...
- [ ] 2.5 Add `AdminChannelStatusView` TypeScript type to `types/admin-config.ts`
    - Match Rust `AdminChannelStatusView` serialization
    - **Acceptance**: Type compiles and matches gateway response
    - **Spec ref**: REQ-9

- [ ] 2.6 Create `ChannelsOverview.vue` component
...
- [ ] 2.7 Create `ChannelsOverview.spec.ts` unit tests
...
- [ ] 3.1 Extend `SecuritySettings.vue` to show autonomy policy details
...
- [ ] 3.2 Extend `SecuritySettings.spec.ts` for new autonomy fields
    - Test rendering with populated and empty lists
    - Test add/remove entry flows
    - **Acceptance**: Tests pass
    - **Spec ref**: REQ-10

- [ ] 3.3 Extend `AdminConfigUpdateRequest` and payload for autonomy policy patches
...
- [ ] 3.4 Add `AdminHealthView` and `ComponentHealth` types to `gateway/admin.rs`
    - Define structs per design interface contracts
    - **Acceptance**: Types compile
    - **Spec ref**: REQ-4

- [ ] 3.5 Implement `handle_admin_health` handler
...
- [ ] 3.6 Register `/web/admin/health` route and add tests
    - **Acceptance**: Route registered; integration tests pass
    - **Spec ref**: REQ-4, REQ-8, REQ-10

- [ ] 3.7 Create `HealthDashboard.vue` component
...
- [ ] 3.8 Create `HealthDashboard.spec.ts` unit tests
    - **Acceptance**: Tests pass
    - **Spec ref**: REQ-10

### 3C: Scheduled Task List

- [ ] 3.9 Add `AdminTaskListView` and `AdminTaskView` types to `gateway/admin.rs`
    - **Acceptance**: Types compile
    - **Spec ref**: REQ-5

- [ ] 3.10 Implement `handle_admin_tasks` handler
...
- [ ] 3.11 Register `/web/admin/tasks` route and add tests
    - **Acceptance**: Route registered; integration tests pass
    - **Spec ref**: REQ-5, REQ-10

- [ ] 3.12 Extend `SchedulerSettings.vue` to include task list
...
- [ ] 3.13 Extend `SchedulerSettings.spec.ts` for task list
    - **Acceptance**: Tests pass
    - **Spec ref**: REQ-10
...
- [ ] 4.1 MCP config endpoint and dashboard section
- [ ] 4.2 Tunnel config endpoint and dashboard section
- [ ] 4.3 Cost tracking endpoint and dashboard section
- [ ] 4.4 Model catalog endpoint and dashboard section
- [ ] 4.5 Daemon status endpoint and dashboard section
...
- [ ] 5.1 Implement `POST /web/chat/stream` SSE endpoint in gateway
...
- [ ] 5.2 Add gateway integration tests for streaming endpoint
...
- [ ] 5.3 Update `useChat.ts` to support SSE streaming
...
- [ ] 5.4 Update `ChatMessage.vue` for streaming state
...
- [ ] 5.5 Add chat app tests for streaming
    - **Acceptance**: Tests pass
    - **Spec ref**: REQ-10

### 5B: Tool Approval

- [ ] 5.6 Implement `POST /web/chat/tool-approval` endpoint in gateway
...
- [ ] 5.7 Implement tool approval delivery via SSE `tool_approval` event
...
- [ ] 5.8 Create `ToolApprovalCard.vue` component
...
- [ ] 5.9 Create `ToolApprovalCard.spec.ts` unit tests
    - **Acceptance**: Tests pass
    - **Spec ref**: REQ-10

### 5C: Session Persistence and Health

- [ ] 5.10 Add session persistence to chat app
...
- [ ] 5.11 Create `HealthIndicator.vue` component
...
- [ ] 5.12 Create `HealthIndicator.spec.ts` unit tests
    - **Acceptance**: Tests pass
    - **Spec ref**: REQ-10
