...
The `AdminConfigView` payload MUST return a comprehensive representation of the current
configuration, matching the expanded scope of the update request, while stripping any sensitive
credentials (e.g., raw API keys).

#### Scenario: Fetching current configuration

- GIVEN an authenticated admin client
- WHEN a GET request is made to the configuration endpoint
- THEN the server returns an `AdminConfigView` object containing all public configuration fields
  from `config.toml`
