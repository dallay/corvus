...
- Staged files MUST be cleaned up via `StagedImageGuard` RAII semantics
- The guard's `Drop` implementation MUST call `StagedImage::cleanup()` for each staged image
- Cleanup MUST be best-effort (log warning on failure, do not panic)
- Cleanup MUST occur on all exit paths: success, error, timeout, early return
