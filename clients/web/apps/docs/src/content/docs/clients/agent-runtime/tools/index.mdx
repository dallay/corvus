...
## Claude-style parity mapping (slice 1)
...
| `TaskCreate` / `TaskGet` / `TaskList` / `TaskUpdate` / `TaskStop` | None in this slice | Deferred | Explicitly out of scope; `schedule` and `cron_*` are not task-lifecycle replacements. |
