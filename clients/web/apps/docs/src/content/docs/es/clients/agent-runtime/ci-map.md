...
Los checks merge-blocking deben mantenerse pequeños y deterministas. Los checks opcionales son útiles para automatización y mantenimiento, pero no deben bloquear desarrollo normal.
...
- `.github/workflows/pr-hygiene.yml` (`PR Hygiene`)
    - Propósito: Nudge a PRs stale-but-active para rebase/re-run de required checks antes de starvation de cola
...
- `Workflow Sanity`: PR/push cuando cambian `.github/workflows/**`, `.github/*.yml`, o `.github/*.yaml`
