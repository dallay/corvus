...
## Mapeo de paridad estilo Claude (slice 1)
...
| `TaskCreate` / `TaskGet` / `TaskList` / `TaskUpdate` / `TaskStop` | Ninguna en este slice | Diferido | Quedan explícitamente fuera de alcance; `schedule` y `cron_*` no sustituyen el ciclo de vida de tareas. |
