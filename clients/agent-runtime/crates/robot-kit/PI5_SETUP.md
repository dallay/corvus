...
Complete guide to setting up a Corvus-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.corvus/models
cp models/ggml-base.bin ~/.corvus/models/
```
...
mkdir -p ~/.corvus/models/piper
cd ~/.corvus/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.corvus/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build Corvus Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/theonlyhennygod/corvus
cd corvus

# Build robot kit
cargo build --release -p corvus-robot-kit

# Build main corvus (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.corvus
nano ~/.corvus/robot.toml
```

```toml
# ~/.corvus/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.corvus/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/corvus_sensors.fifo"
...
```bash
# Configure Corvus to use robot tools
cat > ~/.corvus/config.toml << 'EOF'
...
cp ~/corvus/crates/robot-kit/SOUL.md ~/.corvus/workspace/

# Start agent
./target/release/corvus agent
```
...
# Start sensor loop
if [ ! -p /tmp/corvus_sensors.fifo ]; then
    mkfifo /tmp/corvus_sensors.fifo
fi
...
```bash
# /etc/systemd/system/corvus-robot.service
sudo tee /etc/systemd/system/corvus-robot.service << 'EOF'
[Unit]
Description=Corvus Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/corvus
...
sudo systemctl enable corvus-robot
sudo systemctl start corvus-robot

# Check status
sudo systemctl status corvus-robot
journalctl -u corvus-robot -f  # View logs
```
