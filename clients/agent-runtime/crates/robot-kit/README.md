...
A complete toolkit for building AI-powered robots with Corvus. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build Corvus with robot tools

```bash
# Clone and build
git clone https://github.com/dallay/corvus
cd corvus
cargo build -p corvus-robot-kit --release
```
...
# Copy config
mkdir -p ~/.corvus
cp crates/robot-kit/robot.toml ~/.corvus/
cp crates/robot-kit/SOUL.md ~/.corvus/workspace/
...
# Test in mock mode
./target/release/corvus agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/corvus agent -m "Move forward 1 meter"
```
...
```rust
use corvus_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `corvus`, add a thin adapter that maps this
...
mkdir corvus-robot-kit
cp -r target/release/corvus corvus-robot-kit/
cp -r examples/robot_kit corvus-robot-kit/
cp -r ~/.corvus corvus-robot-kit/dot-corvus

# Include models
mkdir -p corvus-robot-kit/models
cp ~/.corvus/models/ggml-base.bin corvus-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf corvus-robot-kit.tar.gz corvus-robot-kit/

# Copy to USB
cp corvus-robot-kit.tar.gz /media/usb/TarBalls/
```
