//! Flash Corvus Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash Corvus Nucleo firmware. Builds from firmware/corvus-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (corvus's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("corvus-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from corvus repo root.",
            firmware_dir.display()
        );
    }

    println!("Building Corvus Nucleo firmware...");
    ...
    println!("Corvus Nucleo firmware flashed successfully.");
