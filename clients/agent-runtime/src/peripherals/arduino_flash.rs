//! Flash Corvus Arduino firmware via arduino-cli.
//!
...
/// Corvus Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
const FIRMWARE_INO: &str = include_str!("../../firmware/corvus-arduino/corvus-arduino.ino");

const FQBN: &str = "arduino:avr:uno";
const SKETCH_NAME: &str = "corvus-arduino";
...
/// Flash Corvus firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("corvus_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling Corvus Arduino firmware...");
    ...
    println!("Corvus firmware flashed successfully.");
