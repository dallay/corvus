...
impl PrometheusObserver {
    pub fn new() -> Self {
        ...
        let agent_starts = IntCounterVec::new(
            prometheus::Opts::new("corvus_agent_starts_total", "Total agent invocations"),
            &["provider", "model"],
        )
        .expect("valid metric");

        let tool_calls = IntCounterVec::new(
            prometheus::Opts::new("corvus_tool_calls_total", "Total tool calls"),
            &["tool", "success"],
        )
        .expect("valid metric");

        let channel_messages = IntCounterVec::new(
            prometheus::Opts::new("corvus_channel_messages_total", "Total channel messages"),
            &["channel", "direction"],
        )
        .expect("valid metric");

        let heartbeat_ticks =
            prometheus::IntCounter::new("corvus_heartbeat_ticks_total", "Total heartbeat ticks")
                .expect("valid metric");

        let errors = IntCounterVec::new(
            prometheus::Opts::new("corvus_errors_total", "Total errors by component"),
            &["component"],
        )
        .expect("valid metric");
        ...
        let request_latency = Histogram::with_opts(
            HistogramOpts::new(
                "corvus_request_latency_seconds",
        ...
        let queue_depth = GaugeVec::new(
            prometheus::Opts::new("corvus_queue_depth", "Message queue depth"),
            &[],
        )
        .expect("valid metric");
...
mod tests {
    ...
    fn encode_produces_prometheus_text_format() {
        ...
        assert!(output.contains("corvus_agent_starts_total"));
        assert!(output.contains("corvus_tool_calls_total"));
        assert!(output.contains("corvus_heartbeat_ticks_total"));
        assert!(output.contains("corvus_request_latency_seconds"));
    }
    ...
    fn counters_increment_correctly() {
        ...
        let output = obs.encode();
        assert!(output.contains("corvus_heartbeat_ticks_total 3"));
    }
    ...
    fn tool_calls_track_success_and_failure_separately() {
        ...
        let output = obs.encode();
        assert!(output.contains(r#"corvus_tool_calls_total{success="true",tool="shell"} 2"#));
        assert!(output.contains(r#"corvus_tool_calls_total{success="false",tool="shell"} 1"#));
    }
    ...
    fn errors_track_by_component() {
        ...
        let output = obs.encode();
        assert!(output.contains(r#"corvus_errors_total{component="provider"} 2"#));
        assert!(output.contains(r#"corvus_errors_total{component="channels"} 1"#));
    }
