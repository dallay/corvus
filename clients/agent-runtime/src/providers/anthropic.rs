...
mod tests {
    ...
    async fn chat_with_system_fails_without_key() {
        let p = AnthropicProvider::new(None);
        let result = p
            .chat_with_system(Some("You are Corvus"), "hello", "claude-3-opus", 0.7)
            .await;
        assert!(result.is_err());
    }
    ...
    fn chat_request_serializes_with_system() {
        let req = ChatRequest {
            model: "claude-3-opus".to_string(),
            max_tokens: 4096,
            system: Some("You are Corvus".to_string()),
            messages: vec![Message {
                role: "user".to_string(),
                content: "hello".to_string(),
            }],
        ...
        let json = serde_json::to_string(&req).unwrap();
        assert!(json.contains("\"system\":\"You are Corvus\""));
    }
    ...
    #[test]
    fn image_blocks_attached_to_last_user_message() {
        ...
        // Simulate the image injection from chat().
        let images = [staged];
        if let Some(last_user) = native_messages.iter_mut().rfind(|m| m.role == "user") {
            for image in &images {
                let bytes = std::fs::read(&image.temp_path).unwrap();
                let b64 = base64_encode(&bytes);
                last_user.content.push(NativeContentOut::Image {
                    source: ImageSource {
                        source_type: "base64".to_string(),
