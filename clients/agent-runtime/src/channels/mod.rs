...
async fn enforce_strict_memory_validation(
    ...
    let validation = match mem.validate_response(user_query, &candidate, None).await {
        Ok(value) => value,
        Err(error) => {
            tracing::warn!("Channel memory validation failed: {error}");
        ...
        .await
    {
        Ok(value) => value,
        Err(error) => {
            tracing::warn!("Channel ontology correction pass failed: {error}");
            return format!(
    ...
    match mem.validate_response(user_query, &corrected, None).await {
        ...
        Err(error) => {
            tracing::warn!("Channel post-correction ontology validation failed: {error}");
