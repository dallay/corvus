...
Corvus enforces security at **every layer** — not just the sandbox. It passes all items from the
...
> **Run your own nmap:** `nmap -p 1-65535 <your-host>` — Corvus binds to localhost only, so nothing
...
2. **Get your credentials:**
  - **Access Token:** From WhatsApp → API Setup → Generate token (or create a System User for
    permanent tokens)
  - **Phone Number ID:** From WhatsApp → API Setup → Phone number ID
  - **Verify Token:** You define this (any random string) — Meta will send it back during webhook
    verification
