...
// in `~/.corvus/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.corvus/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(corvus_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: corvus_dir.join(".secret_key"),
            enabled,
        }
    }
