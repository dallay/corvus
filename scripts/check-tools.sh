...
if [ -z "$java_major" ] || [ "$java_major" -lt 21 ]; then
  echo "Error: JDK 21+ required. Found: ${java_major:-unknown}"
  exit 1
fi

if [ "$node_major" -lt 22 ]; then
  echo "Error: Node.js 22+ required. Found: $node_major"
  exit 1
fi

if [ "$pnpm_major" -lt 10 ]; then
  echo "Error: pnpm 10+ required. Found: $pnpm_major"
  exit 1
fi

if [ "$rust_major" -lt 1 ] || { [ "$rust_major" -eq 1 ] && [ "$rust_minor" -lt 75 ]; }; then
  echo "Error: Rust 1.75+ required. Found: $rust_full_ver"
  exit 1
fi
